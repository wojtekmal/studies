...
zatem $x = 0$ jest jedynym wspólnym elementem $\ker(p(f))$ i $\ker(q(f))$. Pozostało jeszcze udowodnić, że $\ker(p(f) \circ q(f)) = \ker(p(f)) + \ker(q(f))$. Jeśli $v \in \ker(p(f))$ i $w \in \ker(q(f))$, to $(p(f) \circ q(f))(v) = (q(f) \circ p(f))(v) = 0$ i $(p(f) \circ q(f))(w) = 0$. $p(f) \circ q(f)$ jest liniowe, bo jest złożeniem funkcji liniowych, czyli $(p(f) \circ q(f))(v + w) = 0$, czyli zachodzi $\ker(p(f)) + \ker(q(f)) \subseteq \ker(p(f) \circ q(f))$
\end{rozwiazanie}
